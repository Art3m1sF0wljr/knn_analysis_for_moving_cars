...
CONFIG = {
    'streams': {
        ...
        'stream2': {
            ...
            'active': True,
            'motion_detection': True,
            'output_directory': '/home/art3m1sf0wl/program/street_cars/motion_clips_rear'
        }
...
<script>
    ...
    // Hardcoded stream locations - easy to increment by adding more entries
    const STREAM_CONFIGS = {
        'stream1': '/surveillance/mjpg_feed/stream1',
        'stream2': '/surveillance/mjpg_feed/stream2'
        // Add more streams here as needed:
        // 'stream3': '/surveillance/mjpg_feed/stream3',
        // 'stream4': '/surveillance/mjpg_feed/stream4'
    };
    
    // Initialize streams when page loads
    function initializeStreams() {
        // Initialize streams with staggered delays to avoid browser overload
        const streams = Object.keys(STREAM_CONFIGS);
        
        streams.forEach((streamId, index) => {
            setTimeout(() => {
                console.log('Initializing stream:', streamId);
                initializeStream(streamId, 'mjpg');
            }, index * 1000); // Stagger by 1 second between each stream
        });
    ...
    function initializeStream(streamId, streamType) {
        ...
        const streamUrl = STREAM_CONFIGS[streamId];
        if (!streamUrl) {
            console.error('No stream URL configured for:', streamId);
            status.innerHTML = '<span class="offline">●</span> Stream not configured';
            return;
        }
        
        console.log('Stream URL for', streamId + ':', streamUrl);
        ...
        function loadStream() {
            // Add cache busting
            const url = streamUrl + '?t=' + new Date().getTime();
        ...
        streamImage.onerror = function() {
            console.error('Image error for stream ' + streamId, 'URL:', streamImage.src);
        ...
        streamImage.onload = function() {
            ...
            // Once loaded, set the src to the actual img element
        ...
        // Additional error handling for the main img element
        img.onerror = function() {
            console.error('Main image error for stream ' + streamId, 'SRC:', img.src);
            status.innerHTML = '<span class="offline">●</span> Stream error';
        };
    ...
    function reconnectStream(streamId) {
        ...
        // Force reload using hardcoded URL
        const img = document.getElementById('video-' + streamId);
        const streamUrl = STREAM_CONFIGS[streamId];
        if (streamUrl) {
            const url = streamUrl + '?t=' + new Date().getTime();
    ...
    function startStatusUpdates(streamId) {
        ...
        const updateInterval = setInterval(() => {
            ...
            // Update client count - using basePath for API calls
    ...
    window.addEventListener('beforeunload', function() {
        Object.keys(STREAM_CONFIGS).forEach(streamId => {
            const img = document.getElementById('video-' + streamId);
    ...
    document.addEventListener('visibilitychange', function() {
        if (!document.hidden) {
            ...
            setTimeout(() => {
                Object.keys(STREAM_CONFIGS).forEach(streamId => {
                    const currentType = streamTypes[streamId] || 'mjpg';
                    initializeStream(streamId, currentType);
                });
            }, 1000);
        }
    });
...
class StreamManager:
    ...
    def _distribute_frame(self, frame_bytes):
        """Distribute frame to all connected clients - optimized version"""
        self.last_frame = frame_bytes
        
        # Use list comprehension to avoid holding lock during frame addition
        clients_to_update = []
        with self.clients_lock:
            clients_to_remove = []
    ...
    def _stream_worker(self):
        ...
        # Add stream-specific delay to stagger connections
        if self.stream_id == 'stream2':
            time.sleep(1)  # Stagger stream2 startup
        ...
        while self.running:
            ...
            while self.running and self.cap.isOpened():
                ...
                # Add stream-specific timing
                frame_counter += 1
                if frame_counter % 10 == 0:  # Small sleep every 10 frames
                    time.sleep(0.01)
...
def mjpg_feed(stream_id):
    """
    Stream as MJPEG - Chrome compatible version with unique boundaries
    """
    if stream_id not in stream_managers:
        return "Stream not found", 404
    ...
    def generate_mjpg():
        """Generate proper MJPEG stream with unique boundaries"""
        client = stream_manager.add_client()
        
        try:
            logger.info(f"[{stream_manager.stream_id}] Starting MJPEG stream for Chrome client")
            
            # Use stream-specific boundary to avoid conflicts
            boundary = f'stream_{stream_id}_frame'
            ...
            for frame_bytes in client.get_frames():
                ...
                if frame_count % 100 == 0:
                    logger.debug(f"[{stream_manager.stream_id}] Sent {frame_count} frames")
    ...
    return Response(
        generate_mjpg(),
        mimetype=f'multipart/x-mixed-replace; boundary=stream_{stream_id}_frame',
        headers={
