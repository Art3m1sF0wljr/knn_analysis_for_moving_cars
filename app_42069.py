...
def get_tcp_stream(host='192.168.8.123', port=42069):
    """
    Connect to TCP stream server
    """
    stream_url = f"tcp://{host}:{port}"
    
    print(f"Connecting to TCP stream at {stream_url}...")
    
    # OpenCV VideoCapture for TCP stream
    cap = cv2.VideoCapture(stream_url)
    
    if not cap.isOpened():
        raise Exception(f"Failed to connect to TCP stream at {stream_url}")
    
    print("Successfully connected to TCP stream")
    return cap
...
def main():
    # Configuration
    tcp_host = "192.168.8.123"
    tcp_port = 42069
    output_directory = "motion_clips"
    frame_buffer_size = 5  # Number of frames to keep in buffer
    restart_delay = 5  # Increased delay for TCP connection issues
    
    while True:
        # Initialize TCP stream
        try:
            cap = get_tcp_stream(tcp_host, tcp_port)
            ...
            print(f"Starting TCP stream analysis at {datetime.datetime.now()}")
            print(f"Stream FPS: {fps}")
            
        except Exception as e:
            print(f"Failed to connect to TCP stream: {str(e)}")
        ...
        try:
            while cap.isOpened():
                ret, frame = cap.read()
                if not ret:
                    print("TCP stream ended or connection lost")
                    break
                ...
                #cv2.imshow('TCP Stream Preview', frame)
            ...
            print(f"Error during stream processing: {str(e)}", file=sys.stderr)
        finally:
            cap.release()
            ...
            print(f"Connection lost. Reconnecting in {restart_delay} seconds...")
            time.sleep(restart_delay)
