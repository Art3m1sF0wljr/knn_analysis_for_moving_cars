...
NIGHT_SHUTTER=1000000  # 2 seconds in microseconds
NIGHT_GAIN=20.0
NIGHT_FRAMERATE=3       # 1 fps input
        ...
        if [ -z "$stream_pid" ] || ! kill -0 "$stream_pid" 2>/dev/null; then
            ...
            if [ "$mode" = "night" ]; then
                ...
                ffmpeg -f h264 -r $NIGHT_FRAMERATE -i - \
                    ...
                    -c:a aac \
                    -shortest \
                    -f mpegts "tcp://$TCP_HOST:$TCP_PORT?listen=1&timeout=10000000" &
            else
                rpicam-vid -n -t 0 \
                    --width $WIDTH --height $HEIGHT \
                    --framerate $FRAMERATE \
            ...
            echo "Stream available at: tcp://$TCP_HOST:$TCP_PORT"
        fi
